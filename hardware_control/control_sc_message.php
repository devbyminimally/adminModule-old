<?php 
	$sql_msg = "SELECT hardware_cmd_value FROM join_hardware_cmd WHERE (hardware_type = 'SC' OR hardware_type = 'DC') AND hardware_cmd_name = 'station_id' ";
	$query_msg = mysqli_query($conn,$sql_msg);
	$res_msg = mysqli_fetch_all($query_msg,MYSQLI_BOTH);
?>
<section class="content-header">
    <h1><?=$lang_sc_message; ?></h1>
    <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-home"></i> CONTROL</a></li>
      <li class="active"><?=$lang_sc_message; ?></li>
    </ol>
</section>
...
<?php 
	if(isset($_POST['change_msg'])){
		...
		foreach ($_POST['select_hardware'] as $key => $value_select_hardware) {
			...
			$upload_th_query = mysqli_query($conn,$upload_th_sql);
		...
		//$upload_th_sql = "UPDATE selfcheck SET confic_value = '$msg_pre_th' , last_update_date = now() , last_update_user = '$username' WHERE confic_name = 'sc_msg_th'";
		//$update_th_query = mysqli_query($conn,$upload_th_sql);

		//$upload_en_sql = "UPDATE selfcheck SET confic_value = '$msg_pre_en' , last_update_date = now() , last_update_user = '$username' WHERE confic_name = 'sc_msg_en'";
		//$update_en_query = mysqli_query($conn,$upload_en_sql);
